...
/* top of the stack; defined within linker script */
...
// 1 word stack start address; 15 + 97 = 112 words ISR vector table 
...
void Reset_Handler(void)
{
  ...
  uint32_t size = ((uint32_t)&_edata - (uint32_t)&_sdata)/sizeof(uint32_t);

  uint32_t *pDest = (uint32_t*)&_sdata; // sram
  ...
  size = ((uint32_t)&_ebss - (uint32_t)&_sbss)/sizeof(uint32_t);

  pDest = (uint32_t*)&_sbss; // sram
  for(uint32_t i = 0; i < size; i++) {
     *pDest++ = 0;
  }
